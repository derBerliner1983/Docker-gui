...
const MANAGED_BEGIN = '# >>> core-hub managed (do not edit) >>>';
const MANAGED_END = '# <<< core-hub managed <<<';
...
export async function proxyRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/proxy/ca', { preHandler: requireAuth }, async (_req, reply) => {
    ...
    reply.header('Content-Disposition', 'attachment; filename="core-hub-root-ca.crt"');
