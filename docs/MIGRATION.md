# Container-Migration: Unraid → Core-Hub (manuell)

Diese Anleitung beschreibt, wie du deine Docker-Container samt App-Daten von einem
**Unraid-Server** (oder einem beliebigen anderen Docker-Host) auf **Core-Hub** umziehst.
...
4. [Container in Core-Hub neu anlegen](#4-container-in-core-hub-neu-anlegen)
...
- Auf dem neuen Core-Hub-Server ist **Docker** installiert und läuft
...
| `DST` | Ziel-Pfad auf Core-Hub | `/var/lib/appdata` |
...
Ziel-Verzeichnis auf Core-Hub anlegen (FHS-konform unter `/var/lib`):
...
> Ausgabe auch in eine Datei umleiten und über den **Datei-Manager** in Core-Hub ansehen.
...
Dann die Daten per **rsync** auf Core-Hub holen (resume-fähig, überträgt nur Änderungen):

```bash
# auf dem Core-Hub-Server ausführen
...
## 4. Container in Core-Hub neu anlegen

Jetzt die Container auf Core-Hub neu erstellen – mit denselben Images, Ports, Env-Variablen
und den **lokalen** Volume-Pfaden (`/var/lib/appdata/...` statt `/mnt/user/appdata/...`).

Zwei Wege:

### a) Über die Core-Hub-Oberfläche (empfohlen)
...
1. **Auf dem alten Server** die migrierten Container endgültig stoppen (falls noch aktiv):
   ...
   ssh ALT_USER@ALT_HOST 'docker stop CONTAINER1 CONTAINER2 …'
   ```
2. **Auf Core-Hub** die neuen Container starten (UI: *Start* · CLI: `docker start <name>`)
...
Belegt ein Port auf Core-Hub bereits etwas, zeigt `ss -tlnp` (oder die *Sicherheit*-Seite)
den Konflikt. Entweder den anderen Dienst stoppen oder im Wizard einen anderen Host-Port wählen.

### Rechte-Probleme

Stimmen Eigentümer/Rechte nach dem Kopieren nicht, hilft der **Datei-Manager** in Core-Hub
...
# 4. Container neu anlegen → Core-Hub UI (Container → Neu) oder docker run …

# 5. Alt stoppen, neu starten, prüfen
```

Fertig – deine Dienste laufen jetzt unter Core-Hub. 🎉
