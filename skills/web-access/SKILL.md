---
name: web-access
description: Gibt der KI Internetzugriff über Core-Hub – Websuche und Seitenabruf, um Antworten mit aktuellen Informationen zu belegen. Nutze diese Fähigkeit, wenn eine Frage aktuelle, faktische oder nach dem Trainingsstand liegende Informationen benötigt (Nachrichten, Preise, Wetter, Versionsstände, Öffnungszeiten …).
---

# Web-Zugriff über Core-Hub
...
Über den MCP-Server `core-hub-web` (siehe `mcp/core-hub-web/`):
...
In der Core-Hub-Oberfläche unter *KI-Zentrale → Internetzugriff (KI)* lässt sich
